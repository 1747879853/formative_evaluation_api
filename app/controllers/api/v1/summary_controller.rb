  ...
  def get_summaries_s
    ...
    begin
      ...
      if params.require(:userid).to_i() == -1
        ...
        summaries = sql.select_all "select temp.id,temp.username as name,temp.date,temp.workcontent,SUM(temp.money) as money from (select summaries.id,users.username,summaries.date,summaries.workcontent,costdata.money from summaries left join costdata on costdata.summary_id = summaries.id inner join users on users.id = summaries.user_id where users.id = "+ params.require(:userid) +" and summaries.date between '"+ (Time.now - @date).strftime("%Y-%m-%d") +"' and '"+ Time.now.strftime("%Y-%m-%d") +"') as temp GROUP BY temp.date,temp.username,temp.workcontent,temp.id limit "+limit.to_s()+" offset "+offset.to_s()       
      end      
      if summaries.length > 0
        render json: {total: total,summaries: summaries.as_json}
      end
